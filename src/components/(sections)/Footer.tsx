      ...
      <footer className="flex justify-between w-full max-w-6xl px-4 lg:flex-row flex-col">
        ...
        <nav className="flex gap-4 flex-col lg:flex-row">
          {footerItems.map((item) => (
            ...
            <p
            key={item}
            className="relative inline-block  before:content-[''] before:absolute before:bottom-0 before:left-0 before:h-[1px] before:w-0 before:bg-lime-500 before:transition-all before:duration-300 hover:before:w-[100%] cursor-pointer">
              {item}
            </p>
          ))}
        </nav>
